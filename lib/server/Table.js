...
Table.prototype.startTurnTimer = function(seconds) {
    ...
    this.turnTimer = setTimeout(this.onTurnTimer, seconds * 1e3)
}
...
Table.prototype.onTurnTimer = function() {
    this.ref.transaction(this.processTurnTimeout)
}
...
Table.prototype.processPlayingState = function(current) {
    ...
    // shuffle cards
    current.deck = _.sortBy(_.range(1, 53), function() {
        return Math.random()
    })
